...
const hours = numberArg("hours", 0.8);
...
const defaultRetailFloor = quantity === 1 ? 29.9 : 0;
...
const costs = {
  ...
  individualPackagingPerUnit: includeIndividualPackaging
    ...
    : 0,
  outerShippingSuppliesPerOrder: numberArg("shipping-supplies", 0.85),
  designSetupPerOrder: numberArg("setup", personalized ? 5 : 0),
  failureReservePercent: numberArg("failure", 10),
  overheadPercent: numberArg("overhead", 10),
  targetGrossMarginPercent: numberArg("margin", 35),
  minimumRetailPricePerUnit: numberArg("floor", defaultRetailFloor),
};
...
const marginRoundedPrice = commercialNinety(minimumPricePerUnit);
const recommendedPricePerUnit = money(
  Math.max(marginRoundedPrice, costs.minimumRetailPricePerUnit)
);
const minimumOrderPrice = minimumPricePerUnit * quantity;
const recommendedOrderPrice = recommendedPricePerUnit * quantity;
const grossMarginAtRecommended =
  recommendedPricePerUnit > 0
    ? ((recommendedPricePerUnit - totalCostPerUnit) / recommendedPricePerUnit) * 100
    : 0;
...
const result = {
  ...
  marginRoundedPricePerUnit: marginRoundedPrice,
  recommendedCommercialPricePerUnit: recommendedPricePerUnit,
  grossMarginAtRecommendedPercent: money(grossMarginAtRecommended),
