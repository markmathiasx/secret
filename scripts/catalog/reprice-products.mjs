...
const report = {
  ...
  rule: "pricePix = custo estimado da peca + 30% de lucro; priceCard = pricePix + 1.00",
...
if (apply) {
  for (const item of reportItems) {
    ...
    overrides[item.id] = {
      ...
      pricingSource: "production-cost-plus-30-percent-2026-06-14",
      pricingPolicyBand: item.productType,
      minimumSafePrice: item.minimumSafePrice,
      costBase: item.totalCost,
      profitMode: "markup",
      profitTargetPercent: 30,
      estimatedProfitAmount: Number((item.recommendedPix - item.totalCost).toFixed(2)),
      estimatedProfitPercent: item.recommendedPix > 0 ? Number((((item.recommendedPix - item.totalCost) / item.recommendedPix) * 100).toFixed(2)) : 0,
