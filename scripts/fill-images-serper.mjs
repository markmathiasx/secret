...
const REPORT_PATH = path.join(ROOT, "reports", "fill-images-serper-report.json");
...
const MAX_ITEMS = Number(process.env.FILL_MAX_ITEMS || "50");
...
async function callSerper(query) {
  if (!API_KEY) {
    throw new Error("SERPER_API_KEY não encontrado em .env.local");
  }
...
function pickImageResult(payload) {
  ...
  for (const item of list) {
    const original = String(item.imageUrl || item.image || item.thumbnailUrl || item.thumbnail || "").trim();
    if (!original || !original.startsWith("http")) continue;
    if (looksBlocked(original)) continue;
    return {
      url: original,
      title: String(item.title || "").trim(),
      source: String(item.source || item.link || "").trim()
    };
  }
  return null;
}
...
async function main() {
  ...
  for (const entry of manifest) {
    ...
    try {
      const payload = await callSerper(query);
  ...
  console.log(JSON.stringify({
    ...
    report: "reports/fill-images-serper-report.json"
  }, null, 2));
}
