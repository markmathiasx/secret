...
export function createCommerceAssistantInstructions(channel: AssistantChannel, now?: Date) {
  ...
  return [
    ...
    cardCheckoutReady
      ? "Cartão online disponível no checkout seguro, sempre Pix + R$ 1 por produto."
      : "Quando perguntarem sobre cartão, explique que cada produto fica Pix + R$ 1 e a equipe confirma a forma de pagamento no atendimento humano.",
...
export function buildCommerceFallbackReply(message: string) {
  ...
  if (/(cartao|credito|parcel)/.test(normalized)) {
    return isCardCheckoutConfigured()
      ...
      : `Hoje eu consigo orientar o cartão como Pix + R$ 1 por produto, e a equipe confirma a forma de pagamento no atendimento humano. Se preferir, continue no Pix pelo checkout ${checkoutUrl} ou fale no WhatsApp ${whatsappUrl}.`;
  }
