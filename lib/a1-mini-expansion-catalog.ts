...
function buildExpansionProduct(row: ExpansionRow, index: number): Product {
  ...
  return {
    ...
    complexity: 1.18,
    featured: index < 12,
    description: `${row.shortDescription} Modelo de referência: ${row.sourceTitle}.`,
    tags: row.tags,
    ...
    image: row.image,
    imageAlt: `${row.sourceTitle} — modelo de referência para impressão 3D na Bambu Lab A1 Mini`,
    ...
    pricingMode: "referencia-de-encomenda",
    ...
    mediaProvenance: {
      provider: "MakerWorld",
      sourceTitle: row.sourceTitle,
      sourceProductUrl: row.sourceProductLink,
