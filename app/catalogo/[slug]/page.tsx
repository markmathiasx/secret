...
export default async function ProductPage({
  ...
  const whatsappHref = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Quero comprar ${product.name}. Quantidade: 1. Pix: ${formatCurrency(product.pricePix)}. Cartão + R$ 1: ${formatCurrency(productCardPrice)}. Categoria: ${product.category}. Intenção: compra pela página do produto. Link: ${productUrl}`)}`;
  const customizationHref = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(`Quero personalizar ${product.name}. Quantidade: 1. Pix: ${formatCurrency(product.pricePix)}. Cartão + R$ 1: ${formatCurrency(productCardPrice)}. Categoria: ${product.category}. Intenção: personalização. Link: ${productUrl}`)}`;
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.78fr)]">
        ...
        <div className="mdh-pdp-buybox p-5 md:p-7 lg:sticky lg:top-32 lg:self-start">
          ...
          <div className="mt-6 rounded-[8px] border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/70">
            ...
            <p className="mt-2">Pix {formatCurrency(product.pricePix)}. Cartão + R$ 1 {formatCurrency(productCardPrice)}.</p>
