...
export function CatalogExplorerLoader({ initialProducts, expectedTotal, ...explorerProps }: Props) {
  ...
  useEffect(() => {
    ...
    async function loadCatalog() {
      try {
        ...
        const payload = await response.json() as { total: number; items: Product[] };
        if (!Array.isArray(payload.items) || payload.items.length === 0 ||
            payload.total !== payload.items.length ||
            payload.items.some((item) => !item || typeof item.id !== "string" || !Array.isArray(item.images))) {
  ...
  return (
    <div data-catalog-load-state={status} data-catalog-loaded-count={products.length}>
      ...
      <CatalogExplorer products={products} {...explorerProps} syncUrl={status === "ready"} />
    </div>
  );
}
