...
export function CartRecoveryDock() {
  ...
  const whatsappHref = useMemo(() => {
    const lines = [
      ...
      ...items.map((item) => `- ${item.quantity}x ${item.title}. Pix: ${formatCurrency(item.pricePix)}. Cartão + R$ 1: ${formatCurrency(item.priceCard)}.`),
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-6 text-white/62">
              {totals.quantity} item(ns), Pix {formatCurrency(totals.totalPix)}. Cartão + R$ 1 {formatCurrency(totals.totalCard)}. Salvo às {formatSavedAt(lastUpdatedAt)}.
            </p>
          </div>
        </div>
        <button
