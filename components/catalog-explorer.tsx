...
type PurchaseIntent = 'Geral' | 'Compra rápida' | 'Economia' | 'Presente' | 'Atacado';
...
export function CatalogExplorer({
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [products, query, category, collection, availability, order, priceRange, verifiedOnly, selectedMaterial, customizableOnly, purchaseIntent, economyThreshold]);
    ...
    <div className="space-y-6">
      <div className="glass-panel p-5">
        <div className="grid gap-4 xl:grid-cols-[1.1fr_0.42fr_0.42fr_0.42fr_0.42fr_0.42fr_0.42fr]">
          ...
          <label className="text-sm text-white/70"><span className="mb-2 block">Disponibilidade</span><select value={availability} onChange={(event) => { setAvailability(event.target.value as CatalogAvailability); safeSetPage(1); }} className="field-base"><option>Todos</option><option>Pronta entrega</option><option>Sob encomenda</option></select></label>
          <label className="text-sm text-white/70"><span className="mb-2 block">Material</span><select value={selectedMaterial} onChange={(event) => { setSelectedMaterial(event.target.value); safeSetPage(1); }} className="field-base">{materialOptions.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label className="text-sm text-white/70"><span className="mb-2 block">Intenção</span><select value={purchaseIntent} onChange={(event) => { setPurchaseIntent(event.target.value as PurchaseIntent); safeSetPage(1); }} className="field-base"><option>Geral</option><option>Compra rápida</option><option>Economia</option><option>Presente</option><option>Atacado</option></select></label>
          <label className="text-sm text-white/70"><span className="mb-2 block">Ordenar</span><select value={order} onChange={(event) => setOrder(event.target.value)} className="field-base"><option>Mais Recentes</option><option>Preço</option><option>Nome</option><option>Maior lucro</option><option>Menor prazo</option></select></label>
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-3">
          <label className="text-sm text-white/70"><span className="mb-2 block">Preço</span><input type="range" min={priceLimits.min} max={priceLimits.max} step={5} value={priceRange[0]} onChange={(event) => setPriceRange([Number(event.target.value), priceRange[1]])} className="w-full accent-cyan-400" /><input type="range" min={priceLimits.min} max={priceLimits.max} step={5} value={priceRange[1]} onChange={(event) => setPriceRange([priceRange[0], Number(event.target.value)])} className="w-full accent-cyan-400" /><div className="mt-1 flex justify-between text-xs text-white/60"><span>R$ {priceRange[0]}</span><span>R$ {priceRange[1]}</span></div></label>
          <label className="text-sm text-white/70"><span className="mb-2 block">Quantidade</span><input type="range" min={1} max={30} step={1} value={quantity} onChange={(event) => setQuantity(Number(event.target.value))} className="w-full accent-emerald-400" /><div className="mt-1 flex justify-between text-xs text-white/60"><span>{quantity} un.</span><span>{Math.round(bundleDiscount * 100)}% off</span></div></label>
          <div className="rounded-[18px] border border-emerald-300/20 bg-emerald-300/10 p-3 text-xs text-emerald-100">Simulador de combo ativo. Em lote o catálogo já calcula economia.</div>
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => { setVerifiedOnly((value) => !value); safeSetPage(1); }} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${verifiedOnly ? 'border-emerald-300/35 bg-emerald-300/12 text-emerald-50' : 'border-white/10 bg-white/5 text-white/75'}`}>{verifiedOnly ? 'Somente validados' : 'Mostrar também conceituais'}</button>
          <button type="button" onClick={() => { setCustomizableOnly((value) => !value); safeSetPage(1); }} className={`rounded-full border px-4 py-2 text-sm font-semibold transition ${customizableOnly ? 'border-amber-300/35 bg-amber-300/12 text-amber-50' : 'border-white/10 bg-white/5 text-white/75'}`}>Personalizáveis</button>
          <button type="button" onClick={resetFilters} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/75">Limpar filtros</button>
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-white/60">
          <span>{filtered.length} produtos</span><span className="h-1 w-1 rounded-full bg-white/30" /><span>{readyCount} pronta entrega</span><span className="h-1 w-1 rounded-full bg-white/30" /><span>Página {currentPage} de {totalPages}</span><span className="h-1 w-1 rounded-full bg-white/30" /><span>Busca ativa: {deferredQuery || 'tudo'}</span>
        </div>
      </div>

      {compareProducts.length > 0 ? (
        <div className="rounded-[24px] border border-cyan-300/20 bg-cyan-300/8 p-4">
          <div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-semibold text-cyan-100">Comparador ({compareProducts.length}/3)</p><button type="button" onClick={() => setCompareIds([])} className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs font-semibold text-cyan-100">Limpar</button></div>
          <div className="grid gap-3 md:grid-cols-3">
            {compareProducts.map((product) => <article key={product.id} className="rounded-[18px] border border-white/10 bg-black/20 p-3"><h3 className="line-clamp-2 text-sm font-semibold text-white">{product.name}</h3><p className="mt-1 text-xs text-white/60">{formatCurrency(product.pricePix)} no Pix</p><p className="text-xs text-white/60">Prazo: {product.productionWindow}</p><p className="text-xs text-white/60">Lucro: {formatCurrency(product.estimatedUnitProfit ?? 0)}</p></article>)}
          </div>
        </div>
      ) : null}
      ...
      {(favoriteProducts.length > 0 || recentProducts.length > 0) ? (
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-[20px] border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/55">Favoritos</p><div className="mt-2 flex flex-wrap gap-2">{favoriteProducts.slice(0, 8).map((product) => <Link key={product.id} href={getProductUrl(product)} onClick={() => addRecent(product.id)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/80">{product.name}</Link>)}</div></div>
          <div className="rounded-[20px] border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/55">Vistos recentemente</p><div className="mt-2 flex flex-wrap gap-2">{recentProducts.slice(0, 8).map((product) => <Link key={product.id} href={getProductUrl(product)} onClick={() => addRecent(product.id)} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/80">{product.name}</Link>)}</div></div>
        </div>
      ) : null}
      ...
      {visibleItems.length === 0 ? (
        <div className="rounded-[28px] border border-white/10 bg-white/5 p-8 text-center">
          ...
          <p className="mt-3 text-sm leading-7 text-white/65">Ajuste intenção, material, faixa de preço ou disponibilidade para encontrar peças com mais fit comercial.</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3"><button type="button" onClick={resetFilters} className="btn-secondary">Limpar filtros</button><Link href="/imagem-para-impressao-3d" className="btn-primary">Pedir sob medida</Link></div>
        </div>
      ) : null}

      {totalPages > 1 ? (
        <div className="flex flex-wrap items-center justify-center gap-2">
          <button onClick={() => safeSetPage(currentPage - 1)} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/75 disabled:opacity-50" disabled={currentPage === 1}>Anterior</button>
          {[...Array(totalPages)].map((_, index) => <button key={index} onClick={() => safeSetPage(index + 1)} className={`rounded-full border px-4 py-2 text-sm font-semibold ${currentPage === index + 1 ? 'border-cyan-300/35 bg-cyan-300/12 text-cyan-50' : 'border-white/10 bg-white/5 text-white/75'}`}>{index + 1}</button>)}
          <button onClick={() => safeSetPage(currentPage + 1)} className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white/75 disabled:opacity-50" disabled={currentPage === totalPages}>Próxima</button>
        </div>
      ) : null}
    </div>
  );
