...
export async function POST(request: Request) {
  ...
  if (!adminPassword || !adminConfig.sessionToken || adminConfig.sessionToken === "mdh_troque_este_token_no_env") {
    return NextResponse.json(
      { ok: false, error: "Configure ADMIN_PASSWORD e ADMIN_SESSION_TOKEN no .env.local" },
      { status: 500 }
    );
  }
  ...
  const response = NextResponse.json({ ok: true, redirectTo: adminConfig.hiddenPath });
